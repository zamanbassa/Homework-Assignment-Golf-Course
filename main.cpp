...
static Mesh makeRiverStrip(const vector<vec3>& pts, const vector<float>& widths, float yOff){
    ...
    for(int i = 0; i < n; i++){
        ...
        vec3 perp = vec3(-dir.z, 0.f, dir.x);
...
static constexpr float PLAT_H = 0.85f;
...
static Mesh      mVQuad;
static Mesh      mLeafVQuad;
static Mesh      mRockyBoulder;
static Mesh      mRiver, mRiverBank;
static Mesh      mTaperedCyl;
static Mesh      mThickCyl;
static Mesh      mGableTri;
static Mesh      mRestaurantPlateau;
static Mesh      mTerrainWest;

static GLuint texRock[8];
static GLuint texBark;
static GLuint texPalmCrown[3];
static GLuint texWeed[3];
static GLuint texShrub[2];
static GLuint texConcrete;
static GLuint texAgave;
static GLuint texFlax;
static GLuint texLowPalmCrown;
static GLuint texSign;
static GLFWwindow* gWin = nullptr;

static GLuint hudFontTex = 0;
static GLuint hudVAO     = 0, hudVBO = 0;
static float  hudBuf[8192];
...
static float windmillAngle = 0.f;
...
static Mesh makeWestHillTerrain(){
    const float x0=-49.f, z0=-55.f, w=12.f, d=99.f;
    const int   nx=24,    nz=99;
    auto ss = [](float t) -> float {
        float f = glm::clamp(t, 0.f, 1.f);
        return f * f * (3.f - 2.f*f);
    };
    auto fullH = [&](float x, float z) -> float {
        float fe = ss((-37.f - x)   / 5.f);
        float fs = ss(( 44.f - z)   / 9.f);
        float fn = ss((z - (-55.f)) / 5.f);
...
static Mesh makeRestaurantPlateau(){
    ...
    const float px = 8.5f, pz = 7.5f;
    const float sw = 6.0f;

    const float x0 = cx - px - sw;
    const float z0 = cz - pz - sw;
    const float w  = 2.f*(px + sw);
    const float d  = 2.f*(pz + sw);
...
static void drawLowPalmTree(vec3 base, float height, GLuint crownTex, const mat4& vp){
    const float leanRad = 0.05f;
    {
        ...
        m = glm::rotate(m, -leanRad, {0.f, 0.f, 1.f});
        m = glm::scale(m, {1.f, height, 1.f});
        draw(mThickCyl, m, vp, 11);
    }
    vec3 crown = base + vec3(sinf(leanRad)*height, cosf(leanRad)*height, 0.f);

    const float cw1 = height * 0.80f, ch1 = height * 0.60f;
    const float rad1 = cw1 * 0.30f;
    for(int k = 0; k < 8; k++){
    ...
    for(int k = 0; k < 6; k++){
        ...
        m = glm::rotate(m, ang,   {0.f, 1.f, 0.f});
        m = glm::rotate(m, droop, {0.f, 0.f, 1.f});
...
static void drawWindmill(const mat4& vp)
{
    ...
    const float WM_Z  = -25.0f;
    const float GAP   = 0.60f;
    const float B_ZN  = -26.72f;
    const float B_ZS  = -23.28f;
    const float B_XH  = 1.0f;
    ...
    { mat4 m = glm::translate(mat4(1), {WM_X - 0.95f, 2.0f, WM_Z});
      m = glm::rotate(m, 1.5707963f, {0.f, 0.f, 1.f});
      m = glm::scale(m, {0.10f, 0.60f, 0.10f});
      draw(mCylinder, m, vp, 16); }

    const float bD  = 0.08f;
    const float bW  = 0.32f;
    const float bL  = 1.72f;
...
{
    const float BCX    =  2.0f;
    const float BCZ    = 38.11f;
    const float BANG   = atan2f(-7.f, 4.f);
    const float BL     = 8.0f;
    const float BW     = 2.0f;
    const float H      = 1.2f;
...
static void renderBridge(const mat4& vp)
{
    ...
    const float BX_C   = (BX_W + BX_E) * 0.5f;
    const float BZ_N   = -26.0f;
    const float BZ_S   = -16.0f;
    const float BZ_C   = (BZ_N + BZ_S) * 0.5f;
    const float BZ_LEN = BZ_S - BZ_N;
    const float BW     = BX_E - BX_W;
    const float H      = 1.5f;
    const float POST_R = 0.13f;
    const float STEP_H = 0.30f;
    const float STEP_D = 0.40f;
    ...
    auto rpost = [&](float px, float pz) {
        mat4 m = glm::translate(mat4(1), {px, H, pz});
    ...
    for (int s = 0; s < NSTEPS; s++) {
        float zc = BZ_S + (NSTEPS - s - 0.5f) * STEP_D;
        float h  = (s + 1) * STEP_H;
    ...
    for (int s = 0; s < NSTEPS; s++) {
        float zc = BZ_N - (NSTEPS - s - 0.5f) * STEP_D;
        float h  = (s + 1) * STEP_H;
        mat4 m = glm::translate(mat4(1), {BX_C, h * 0.5f, zc});
        m = glm::scale(m, {BW, h, STEP_D});
        draw(mBox, m, vp, 6);
...
static void drawRestaurant(const mat4& vp, float yOff = 0.f){
    ...
    const float slopeA  = atan2f(roofH, roofW * 0.5f);
    const float panelL  = hypotf(roofW * 0.5f, roofH);
    const float panelCx = roofW * 0.25f;
    const float panelCy = roofY + roofH * 0.5f;
...
int main(){
    ...
    // river meshes
    {
        ...
        mRiverBank = makeRiverStrip(pts, bw, 0.001f);
        mRiver     = makeRiverStrip(pts, ww, 0.003f);
    }
    ...
    while(!glfwWindowShouldClose(gWin)){
        ...
        { mat4 m = glm::translate(mat4(1), {14.f, -0.3f, 16.f});
          ...
          draw(mSphere, m, vp, 26); }

        draw(mRiverBank, mat4(1), vp, 23);
        draw(mRiver,     mat4(1), vp,  5);
        ...
        drawPalmTree({-25.13f,0.f,-14.68f},5.5f,texBark,texPalmCrown[0],vp);
        ...
        drawLowPalmTree({  6.5f, PLAT_H, -6.2f}, 3.0f, texPalmCrown[0], vp);
        ...
        drawLowPalmTree({ 29.80f, 0.f, 17.61f}, 3.0f, texPalmCrown[0], vp);
        ...
        drawLowPalmTree({ 5.95f, 0.f, 12.30f}, 2.8f, texPalmCrown[1], vp);
        drawLowPalmTree({ 24.f, 0.f,  24.f}, 3.0f, texPalmCrown[1], vp);
        drawLowPalmTree({ 17.5f, 0.f, 37.f}, 2.7f, texPalmCrown[0], vp);
        ...
        // walkways
        drawWalkway({-33.f,0.f,29.f}, {-33.f,0.f,25.f}, 1.5f, vp);
        drawWalkway({-33.f,0.f,11.f}, {-33.f,0.f, 6.f}, 1.5f, vp);
        drawWalkway({-33.f,0.f,-8.f}, {-35.f,0.f,-13.f}, 1.2f, vp);
        drawWalkway({-36.f,0.f,-22.f},{-33.f,0.f,-27.f}, 1.2f, vp);
        drawWalkway({-33.f,0.f,-36.f},{-28.f,0.f,-41.f}, 1.5f, vp);
        drawWalkway({-20.f,0.f,-47.f},{-12.f,0.f,-47.f}, 1.2f, vp);
        drawWalkway({ 7.f, 0.f,-47.f},{ 14.f,0.f,-47.f}, 1.2f, vp);
        drawWalkway({ 21.f, 0.f,-46.5f},{25.f,0.f,-45.5f}, 1.2f, vp);
        drawWalkway({ 31.f, 0.f,-45.5f},{34.f,0.f,-39.f},  1.2f, vp);
        drawWalkway({  5.0f,0.f, 0.f},{ 10.f,0.f,  0.f}, 1.5f, vp);
        drawWalkway({  0.f, 0.f,-4.0f},{  0.f,0.f,-7.0f},1.5f, vp);
        drawWalkway(gHoles[1]->getTeePos() + vec3( 5.f,0.f, 2.f), {-3.f,0.f,5.f}, 2.0f, vp);
        drawWalkway(gHoles[6]->getTeePos() + vec3( 7.f,0.f, 3.f), {-3.f,0.f,5.f}, 2.0f, vp);
