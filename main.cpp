...
static float hillH(float x, float z)
{
    ...
    {
        float f = (t < r) ? t / r : (t > 1.f - r) ? (1.f - t) / r
                                                  : 1.f;
    ...
    for (int iz = 0; iz <= nz; iz++)
    {
        for (int ix = 0; ix <= nx; ix++)
        {
            ...
            auto fh = [&](float fx, float fz) -> float
            {
                ...
                return terrainH(fx, fz) * edgeFade(glm::clamp(px, 0.f, 1.f)) * edgeFade(glm::clamp(pz, 0.f, 1.f));
            };
...
{
    cam.fov -= glm::clamp((float)dy * 2.5f, 10.f, 95.f);
}
...
static void cbKey(GLFWwindow *w, int key, int, int act, int)
{
    ...
    case GLFW_KEY_KP_SUBTRACT:
        ...
        break;
    case GLFW_KEY_T:
        todSpeed = (todSpeed > 0) ? 0 : 0.005f;
        break;
    case GLFW_KEY_EQUAL:
        cam.fov = glm::max(cam.fov - 3.f, 10.f);
        break;
    case GLFW_KEY_MINUS:
        cam.fov = glm::min(cam.fov + 3.f, 95.f);
        break;

    // ── Spotlight direction (arrow keys) ──
    case GLFW_KEY_UP:
        spotPitOff = glm::clamp(spotPitOff - 0.1f, -1.4f, 0.f);
        break;
    case GLFW_KEY_DOWN:
        spotPitOff = glm::clamp(spotPitOff + 0.1f, -1.4f, 0.f);
        break;
...
static void drawChair(vec3 pos, float rot, const mat4 &vp)
{
    ...
    {
        mat4 m = glm::translate(base, {0, 0.68f, 0});
        m = glm::scale(m, {0.48f, 0.06f, 0.44f});
        draw(mBox, m, vp, 6);
    }
    // Backrest: centred at y=1.00, offset behind seat (local -z)
    {
        mat4 m = glm::translate(base, {0, 1.00f, -0.19f});
        m = glm::scale(m, {0.48f, 0.60f, 0.06f});
        draw(mBox, m, vp, 6);
    }
}
...
static void drawTable(vec3 pos, const mat4 &vp)
{
    ...
    {
        mat4 m = glm::translate(mat4(1), pos + vec3(0, 1.18f, 0));
        m = glm::scale(m, {1.2f, 0.06f, 0.90f});
        draw(mBox, m, vp, 6);
    }
}
...
int main()
{
    ...
    for (int i = 0; i < 3; i++)
        texPalmCrown[i] = loadTexture(crownPaths[i]);
    ...
    while (!glfwWindowShouldClose(gWin))
    {
        ...
        {
            mat4 m = glm::translate(mat4(1), {0.f, 0.005f, 6.f});
            m = glm::scale(m, {16.f, 1.f, 4.f});
            drawWithTex(mQuad, m, vp, 8, texConcrete);
        }
        // Back slab: z=-4..-7
        {
            mat4 m = glm::translate(mat4(1), {0.f, 0.005f, -5.5f});
            m = glm::scale(m, {16.f, 1.f, 3.f});
            drawWithTex(mQuad, m, vp, 8, texConcrete);
        }
        // Left slab: x=-5..-8
        {
            mat4 m = glm::translate(mat4(1), {-6.5f, 0.005f, 0.f});
            m = glm::scale(m, {3.f, 1.f, 8.f});
            drawWithTex(mQuad, m, vp, 8, texConcrete);
        }
        // Right slab: x=5..9 (width 4 to contain right seating)
        {
            mat4 m = glm::translate(mat4(1), {7.f, 0.005f, 0.f});
            m = glm::scale(m, {4.f, 1.f, 8.f});
            drawWithTex(mQuad, m, vp, 8, texConcrete);
        }
        ...
        {
            mat4 m = glm::translate(mat4(1), {-6.5f, 0.02f, 7.5f});
            m = glm::scale(m, {1.6f, 1.f, 1.6f});
            draw(mQuad, m, vp, 23);
        }
        ...
        {
            mat4 m = glm::translate(mat4(1), {8.5f, 0.02f, 3.5f});
            m = glm::scale(m, {1.6f, 1.f, 1.6f});
            draw(mQuad, m, vp, 23);
        }
    ...
    freeMesh(mCircle);
    freeMesh(mVQuad);
    freeMesh(mRockyBoulder);
    freeMesh(mTerrainHills);
