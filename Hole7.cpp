    ...
    : Hole(7, q, b, c, t)
{
    ...
    // Arc1 (south bump): PI→0, south semicircle
    ...
    // Arc2 (north bump): PI→2*PI, north semicircle
...
void Hole7::render(const mat4& vp) const {
    ...
    const float c2x = H7_X0+H7_LS+3.f*H7_RA;     //  2  (north-bump centre)
    const float cz  = H7_Z0;
    const float teeX= c1x - H7_RA;               // -13  west opening
    const float cupX= c2x + H7_RA;               //   7  east opening
...
void Hole7::wallCollide(vec3& pos, vec3& vel) const {
    ...
    // Radial collision on nearest arc centre
