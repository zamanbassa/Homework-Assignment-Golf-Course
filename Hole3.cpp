...
{
    // South loop: gap at -π/2 pointing toward north loop
    mWall1 = makeArcWall(H3_R, H3_WTH3, H3_WH3, -PI3/2.f+H3_GAP, -PI3/2.f+2.f*PI3-H3_GAP, 56);
    // North loop: gap at +π/2 pointing toward south loop
...
void Hole3::render(const mat4& vp) const {
    ...
    float lc1z = cz + H3_R*0.9f;   // south loop centre z
    float lc2z = cz - H3_R*0.9f;   // north loop centre z
