...
static const float H9_TEE_X  = (H9_WX1 + H9_WX2) * 0.5f;
static const float H9_CUP_X  = (H9_EX1 + H9_EX2) * 0.5f;
static const float H9_PLAY_Z = H9_ZN - 1.5f;

Hole9::Hole9(const Mesh* q, const Mesh* b, const Mesh* c, const Mesh* t)
    : Hole(9, q, b, c, t) {}

void Hole9::render(const mat4& vp) const {
    const float armW    = H9_WX2 - H9_WX1;
    const float armL    = fabsf(H9_ZS - H9_ZN);
    const float connW   = H9_EX2 - H9_WX1;
    const float connL   = fabsf(H9_ZB - H9_ZS);
    const float armMidZ  = (H9_ZN + H9_ZS) * 0.5f;
    const float connMidZ = (H9_ZS + H9_ZB) * 0.5f;
    const float connMidX = (H9_WX1 + H9_EX2) * 0.5f;
    ...
    // divider
    const float divW = H9_EX1 - H9_WX2;
